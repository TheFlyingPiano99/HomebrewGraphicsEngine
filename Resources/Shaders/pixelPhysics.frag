...
void main()
{
	...
	if (isEqual(current, water)) {
		bool isGrounded = checkIsGrounded(currentPos);
		if(!isGrounded) {				// fall down
			if (!isEqual(up, water) && !(isEqual(left, water) && checkIsGrounded(leftPos))
				&& !(isEqual(right, water) && !isEqual(rightRight, empty) && checkIsGrounded(rightPos))) {	
				color = empty;
			}
		}
		else {
			if (!isEqual(up, water) && !isEqual(left, water) && isEqual(right, water)) {
				color = empty;
			}
			else if (
				(!isEqual(up, water) || isEqual(rightUp, empty) && !isEqual(rightUpUp, water) || isEqual(leftUp, empty) && !isEqual(leftUpUp, water))  
				&& isEqual(right, empty) && !isEqual(rightUp, water)
				) {	// flow right
				color = empty;
			}
			else if (
				(!isEqual(up, water) || isEqual(rightUp, empty) && !isEqual(rightUpUp, water) || isEqual(leftUp, empty) && !isEqual(leftUpUp, water)) 
				&& isEqual(left, empty) && !(isEqual(leftLeft, water) && checkIsGrounded(leftLeftPos) && !isEqual(leftUp, water))
			) {	// flow left
				color = empty;
			}
		}
	...
	// Around particle:
		...
		if (isEqual(up, water)) {	// fall down
			color = water;
		}
		else if (isEqual(left, water) && checkIsGrounded(leftPos)) {	// flow right
			color = water;
		}
		else if (isEqual(right, water) && checkIsGrounded(rightPos) && !(isEqual(rightRight, empty) && !isEqual(rightRightUp, water))) {	// flow left
			color = water;
		}
	}
