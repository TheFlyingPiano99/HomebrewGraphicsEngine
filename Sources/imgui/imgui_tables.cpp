...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    {
        const ImGuiID context_menu_id = ImHashStr("##ContextMenu", 0, table->glID);
...
void    ImGui::EndTable()
{
    ...
    // Pop from id stack
    IM_ASSERT_USER_ERROR(inner_window->IDStack.back() == table->glID + table->InstanceCurrent, "Mismatching PushID/PopID!");
...
{
    IM_ASSERT(column_n >= 0 && column_n < table->ColumnsCount);
    ImGuiID id = table->glID + 1 + (instance_no * table->ColumnsCount) + column_n;
    return id;
}
...
ImGuiTableSettings* ImGui::TableGetBoundSettings(ImGuiTable* table)
{
    if (table->SettingsOffset != -1)
    {
        ...
        IM_ASSERT(settings->glID == table->glID);
        if (settings->ColumnsCountMax >= table->ColumnsCount)
            return settings; // OK
        settings->glID = 0; // Invalidate storage, we won't fit because of a count change
    }
    return NULL;
}
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    if (settings == NULL)
    {
        settings = TableSettingsCreate(table->glID, table->ColumnsCount);
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    if (table->SettingsOffset == -1)
    {
        settings = TableSettingsFindByID(table->glID);
        if (settings == NULL)
            return;
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        if (settings->glID == 0) // Skip ditched settings
            continue;
        ...
        buf->appendf("[%s][0x%08X,%d]\n", handler->TypeName, settings->glID, settings->ColumnsCount);
...
void ImGui::TableRemove(ImGuiTable* table)
{
    ...
    g.Tables.Remove(table->glID, table);
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    ImFormatString(p, buf_end - p, "Table 0x%08X (%d columns, in '%s')%s", table->glID, table->ColumnsCount, table->OuterWindow->Name, is_active ? "" : " *Inactive*");
...
{
    if (!TreeNode((void*)(intptr_t)settings->glID, "Settings 0x%08X (%d columns)", settings->glID, settings->ColumnsCount))
        return;
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    IM_ASSERT(g.ActiveId == columns->glID + ImGuiID(column_index));
...
ImGuiOldColumns* ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
    ...
    for (int n = 0; n < window->ColumnsStorage.Size; n++)
        if (window->ColumnsStorage[n].glID == id)
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            const ImGuiID column_id = columns->glID + ImGuiID(n);
